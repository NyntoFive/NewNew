...
def load_urls(url=None):
    if url and url is None:
        print("Need a sitemap URL!")
        return None
    session = HTMLSession()
...
def load_file(file="smproducts.xml"):
    with open(file) as f:
        txt = f.read()
        page = HTML(html=txt)
    tmp={}
    tmp['urls'] = page.xpath('//loc/text()')
    tmp['stamps'] = page.xpath('//lastmod/text()')
    return list(zip(tmp['urls'],tmp['stamps']))
